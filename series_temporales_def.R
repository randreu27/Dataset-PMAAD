...
write.csv(df, file = '~/GitHub/Dataset-PMAAD/data_preprocesada.csv', row.names = FALSE)

#Transformación variable temporal 'fecha en la que se publicarón los libros'
rutaDatos <- "~/GitHub/Dataset-PMAAD/data_preprocesada.csv"
...
#Es correcto, hemos verificado que tienen 9541 filas.     <<<<<<<<<<<<<<<<<<<<<<<<<<< PREGUNTAR????
...
# Seleccionar solo las columnas numéricas y de fecha
data_numeric <- select_if(data, is.numeric)

# Ahora, data_numeric contiene todas las columnas numéricas y de fecha de data


# Calculamos la matriz de distancias DTW
distMatrix <- proxy::dist(data_numeric, method = "DTW")

# Realizar el clustering jerárquico
hc <- hclust(distMatrix)

# Realizar el corte en el dendrograma para obtener el número de clusters
k <- cutree(hc, k = 5)

# Añadir el número de cluster a los datos originales
data$cluster <- as.factor(k)

# Dividir los datos en subconjuntos según el cluster
clustered_data <- split(data, data$cluster)

# Plot de las series temporales para cada cluster
for (i in 1:length(clustered_data)) {
  # Obtener las series temporales para el cluster i
  cluster_series <- clustered_data[[i]]
  
  # Calcular la media de los precios para cada fecha
  mean_prices <- aggregate(price ~ publishDate, cluster_series, mean)
  
  # Convertir mean_prices en una serie de tiempo (time series)
  time_series <- xts(mean_prices$price, order.by = mean_prices$publishDate)
  
  # Graficar la serie de tiempo del cluster
  plot(time_series, main = paste("Cluster", i), ylab = "Precio", xlab = "Fecha")
}
